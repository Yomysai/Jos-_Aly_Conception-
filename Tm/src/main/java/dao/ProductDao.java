...
//	}
	...
	public ArrayList<Product> findSponsoredProducts() {
	    ArrayList<Product> listSponsoredProducts = new ArrayList<>();
	    try {
	        sql = connect.prepareStatement("SELECT p.*, d.*, vat.value AS vat_value FROM product p "
	        		+ "LEFT JOIN product_discount pd ON p.id = pd.id_product "
	        		+ "LEFT JOIN discount d ON pd.id_discount = d.id "
	        		+ "INNER JOIN vat ON p.id_vat = vat.id "
	        		+ "WHERE p.sponsoring IS NOT NULL "
	        ...
	        while(rs.next()) {
	            ...
	            listSponsoredProducts.add(product);
